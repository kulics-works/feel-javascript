...
packageControlSubStatement:
	id (left_paren id right_paren)? left_brace (
		functionSupportStatement
	)+ right_brace;
// 定义包事件
packageEventStatement:
	id Colon left_brack Question right_brack nameSpaceItem end;
...
// 函数
implementFunctionStatement: (annotationSupport)? (n = '_')? id (
		templateDefine
	)? parameterClauseIn t = (Right_Arrow | Right_Flow) New_Line* parameterClauseOut left_brace (
		functionSupportStatement
	)* right_brace end;
// 定义控制
implementControlStatement: (annotationSupport)? (n = '_')? id Colon typeType Right_Arrow (
		packageControlSubStatement
	)+ end;

// 重载
overrideStatement:
	parameterClauseSelf left_paren id right_paren Right_Arrow New_Line* left_brace (
...
// 函数
overrideFunctionStatement: (annotationSupport)? (n = '_')? id (
		templateDefine
	)? parameterClauseIn t = (Right_Arrow | Right_Flow) New_Line* parameterClauseOut left_brace (
		functionSupportStatement
	)* right_brace end;
// 定义控制
overrideControlStatement: (annotationSupport)? (n = '_')? id Colon typeType Right_Arrow (
		packageControlSubStatement
	)+ end;

// 协议
protocolStatement: (annotationSupport)? id (templateDefine)? Left_Arrow left_brace (
...
caseDefaultStatement:
	Discard left_brace (functionSupportStatement)* right_brace;
// 条件声明
caseExprStatement: (expression | (id)? Colon typeType) left_brace (
		functionSupportStatement
	)* right_brace;
// 判断条件声明
...
// if 判断
judgeIfStatement:
	Question expression left_brace (functionSupportStatement)* right_brace;
// else if 判断
judgeElseIfStatement:
	expression left_brace (functionSupportStatement)* right_brace;
// 循环
loopStatement:
	iteratorStatement At id left_brace (functionSupportStatement)* right_brace end;
// 集合循环
loopEachStatement:
	expression At (Left_Brack id Right_Brack)? id left_brace (
		functionSupportStatement
	)* right_brace end;
// 条件循环
loopCaseStatement:
	At expression left_brace (functionSupportStatement)* right_brace end;
// 无限循环
loopInfiniteStatement:
	At left_brace (functionSupportStatement)* right_brace end;
// 跳出循环
...
// 检查
checkStatement:
	Bang left_brace (functionSupportStatement)* right_brace (
		checkErrorStatement
	)* checkFinallyStatment end
	| Bang left_brace (functionSupportStatement)* right_brace (
		checkErrorStatement
	)+ end;
// 定义检查变量
usingStatement:
	expression Bang expression (Colon typeType)? end;
// 错误处理
checkErrorStatement: (id | id Colon typeType) left_brace (
		functionSupportStatement
	)* right_brace;
// 最终执行
